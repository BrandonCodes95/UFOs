...
//Creat function to make table interactive 
...
function handleClick() {
    ...
    if (date) {
        //Apply filter to the table data to only keep the rows where the date time values match the filter value 
        filteredData = filteredData.filter(row => row.datetime === date); 
}

    //rebuild the table using the filtered data 
    //NOTE: if no date was entered, then filterData will just be the original tableData
